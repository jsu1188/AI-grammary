...
class ResultPanel(QWidget):
    ...
    def build_ui(self):
        ...
        self.selection_mode_checkbox = QCheckBox("선택 영역 인식 사용")
        self.selection_mode_checkbox.setObjectName("settingsCheck")
        ...
        self.selection_mode_checkbox.toggled.connect(self._sync_input_mode_checks)
        ...
        self.selection_mode_checkbox.toggled.connect(self._update_replace_mode_availability)
    ...
    def _create_text_box(self, placeholder):
        ...
        text_font = QFont("Malgun Gothic", 10)
        text_box.setFont(text_font)
        text_box.document().setDefaultFont(text_font)
    ...
    def _create_prompt_page(self):
        ...
        self.prompt_no_btn = QPushButton("아니오")
    ...
    def _create_settings_tab(self):
        ...
        section.addSpacing(4)
        section.addWidget(self.selection_mode_checkbox)
        section.addStretch()
    ...
    def _create_account_verify_page(self):
        ...
        guide = QLabel("계정 관리 기능을 사용하려면 비밀번호를 한 번 더 입력해 주세요.")
    ...
    def _create_history_page(self):
        ...
        top_row.addStretch()
        self.history_delete_all_btn = QPushButton("전체 삭제")
        self.history_delete_all_btn.setObjectName("dangerButton")
        self.history_delete_all_btn.setFixedHeight(34)
        self.history_delete_all_btn.setFixedWidth(92)
        top_row.addWidget(self.history_delete_all_btn)
    ...
    def show_history_list(self, feature_type, logs):
        ...
        self.history_delete_all_btn.show()
        ...
        for log in logs:
            row = QFrame()
            row.setObjectName("historyListItem")
            row_layout = QHBoxLayout(row)
            row_layout.setContentsMargins(10, 6, 10, 6)
            row_layout.setSpacing(10)
            button = QPushButton(self._history_list_label(log))
            button.setObjectName("historyListItemButton")
            button.setMinimumHeight(58)
            button.clicked.connect(lambda checked=False, item=log: self.show_history_detail(feature_type, item))
            row_layout.addWidget(button, 1)
            if log.get("request_id") is not None:
                delete_button = QPushButton("삭제")
                delete_button.setObjectName("dangerButton")
                delete_button.setFixedWidth(62)
                delete_button.setMinimumHeight(44)
                delete_button.clicked.connect(lambda checked=False, item=dict(log): self._run_history_delete(item))
    ...
    def show_history_detail(self, feature_type, log):
        ...
        self.history_delete_all_btn.hide()
        self._disconnect_history_back()
        ...
        missing_actions = self._missing_request_actions(log) if log.get("request_id") is not None else []
        if missing_actions:
            row_label = QLabel("미호출 기능")
            row_label.setObjectName("historyFieldLabel")
            layout.addWidget(row_label)
            action_row = QHBoxLayout()
            action_row.setSpacing(8)
            tone_input_row = QWidget()
            tone_input_layout = QHBoxLayout(tone_input_row)
            tone_input_layout.setContentsMargins(0, 0, 0, 0)
            tone_input_layout.setSpacing(8)
            tone_request_input = QLineEdit()
            tone_request_input.setObjectName("toneInput")
            tone_request_input.setPlaceholderText("원하는 문체/말투")
            tone_request_submit_btn = QPushButton("실행")
            tone_request_submit_btn.setObjectName("secondaryButton")
            tone_request_submit_btn.clicked.connect(
                lambda checked=False, item=dict(log), input_widget=tone_request_input: self._run_history_request_action(
                    "tone",
                    item,
                    input_widget.text().strip(),
                )
            )
            tone_request_input.returnPressed.connect(tone_request_submit_btn.click)
            tone_input_layout.addWidget(tone_request_input, 1)
            tone_input_layout.addWidget(tone_request_submit_btn)
            tone_input_row.hide()
            for action_key, action_label in missing_actions:
                button = QPushButton(action_label)
                button.setObjectName("secondaryButton")
                if action_key == "tone":
                    button.clicked.connect(
                        lambda checked=False, row=tone_input_row, input_widget=tone_request_input: self._show_history_tone_input(
                            row,
                            input_widget,
                        )
                    )
                    ...
                    button.clicked.connect(
                        lambda checked=False, item=dict(log), key=action_key: self._run_history_request_action(key, item)
                    )
                action_row.addWidget(button)
            action_row.addStretch()
            layout.addLayout(action_row)
            if any(action_key == "tone" for action_key, _ in missing_actions):
                layout.addWidget(tone_input_row)
        self.history_scroll.setWidget(content)

    def close_history_page(self):
        self._restore_main_page()
    ...
    def _resolve_prompt(self, callback):
        ...
        self.prompt_no_btn.setText("아니오")
        return_index = getattr(self, "_return_stack_index", 0)
        if return_index == 0:
            self._restore_main_page()
        else:
            self.content_stack.setCurrentIndex(return_index)
        if callback:
            callback()
    ...
    def _handle_tab_changed(self, index):
        if index != getattr(self, "history_tab_index", -1):
            self._last_non_history_tab_index = index

    def _restore_main_page(self):
        self.content_stack.setCurrentIndex(0)
        history_tab_index = getattr(self, "history_tab_index", -1)
        if self.tabs.currentIndex() == history_tab_index:
            target_index = getattr(self, "_last_non_history_tab_index", 0)
            if target_index == history_tab_index or target_index < 0:
                target_index = 0
    ...
    def _history_detail_fields(self, feature_type, log):
        ...
        if feature_type == 2:
            return [
                ...
                ("맞춤법 검사 결과", log.get("spelling_feedback"), True),
                ("맞춤법 수정된 글", log.get("output_text"), True),
                ("추천 제목", log.get("title"), False),
                ("종류", log.get("feature_type"), False),
                ("저장 시간", created_at, False),
            ]
        ...
        return [
            ("원본 글", log.get("input_text"), True),
            ("제시한 어투/말투", log.get("tone"), False),
            ("어투/말투 결과", log.get("output_text"), True),
            ("추천 제목", log.get("title"), False),
            ("종류", log.get("feature_type"), False),
            ("저장 시간", created_at, False),
        ]
    ...
    def _history_detail_fields(self, feature_type, log):
        ...
        if feature_type == 2:
            return [
                ("원본 글", log.get("input_text"), True),
                ("맞춤법 결과", log.get("spelling_feedback"), True),
                ("맞춤법 교정문", log.get("output_text"), True),
                ("추천 제목", log.get("title"), False),
                ("종류", log.get("feature_type"), False),
                ("저장 시간", created_at, False),
            ]
    ...
    def _request_history_detail_fields(self, log):
        fields = [
            ("원문", log.get("input_text"), True),
            ("원문 저장 시간", self._format_created_at(log.get("created_at", "")), False),
        ]
        spelling = log.get("spelling") or {}
        if spelling:
            fields.extend([
                    ("맞춤법 피드백", spelling.get("spelling_feedback"), True),
                    ("맞춤법 교정문", spelling.get("corrected_text"), True),
            ])
        summary = log.get("summary") or {}
        if summary:
            fields.extend([("요약문", summary.get("summary_text"), True)])
        tone = log.get("tone") or {}
        if tone:
            fields.extend([
                ("문체/말투 요청", tone.get("requested_tone"), False),
                ("문체/말투 결과", tone.get("changed_text"), True),
            ])
    ...
    def _missing_request_actions(self, log):
        if not isinstance(log, dict) or log.get("request_id") is None:
            return []
        actions = []
        if not log.get("spelling"):
            actions.append(("spelling", "맞춤법"))
        if not log.get("summary"):
            actions.append(("summary", "요약"))
        if not log.get("tone"):
            actions.append(("tone", "문체/말투"))
        if not log.get("evaluation"):
            actions.append(("evaluation", "평가"))
        if not log.get("title"):
            actions.append(("title", "제목"))
        return actions

    def _show_history_tone_input(self, row_widget, input_widget):
        row_widget.show()
        input_widget.setFocus()
        input_widget.selectAll()

    def _run_history_request_action(self, action_key, log, tone_text=None):
        callback = getattr(self, "history_request_action_callback", None)
        if callable(callback):
            callback(action_key, log, tone_text)

    def _run_history_delete(self, log):
        callback = getattr(self, "history_delete_callback", None)
        if callable(callback):
            callback(log)
    ...
    def apply_theme(self):
        ...
        self.setStyleSheet(
            ...
            QPushButton#historyListItemButton {{
                background: transparent;
                color: {colors["text"]};
                border: none;
                border-radius: 8px;
            ...
            QPushButton#historyListItemButton:hover {{
                background: {colors["button_bg"]};
            }}
    ...
    def set_account_info(self, account):
        ...
        self.account_password_input.setPlaceholderText("새 비밀번호 입력")
    ...
    def set_input_mode(self, mode):
        normalized = mode if mode in {"clipboard", "realtime", "selection"} else "realtime"
        ...
        self.selection_mode_checkbox.setChecked(normalized == "selection")
    ...
    def _sync_input_mode_checks(self):
        ...
        if not any(checkbox.isChecked() for checkbox in checkboxes):
            fallback_checkbox = self.realtime_mode_checkbox
    ...
    def _render_original_text(self, previous_text=""):
        ...
        self.text_box.clear()
        self._apply_text_document_font(self.text_box)
    ...
    def set_spell_result(self, text):
        self._apply_text_document_font(self.spell_box)
        self.spell_box.setPlainText(text)

    def set_summary_result(self, text):
        self._apply_text_document_font(self.summary_box)
        self.summary_box.setPlainText(text)

    def set_tone_result(self, text):
        self._apply_text_document_font(self.tone_box)
        self.tone_box.setPlainText(text)

    def _apply_text_document_font(self, text_box):
        font = QFont("Malgun Gothic", 10)
        text_box.setFont(font)
        text_box.document().setDefaultFont(font)
