...
pyperclip = _optional_import("pyperclip")
...
class BasePollingReader:
    ...
    def __init__(self, debug: bool = False):
        ...
        self._last_selection_info: dict = {}
    ...
    def read_selection_info(self) -> dict:
...
class NotepadReader(BasePollingReader):
    ...
    def _best_editor_handle(self, root_hwnd: int) -> int | None:
        focused = self._focused_child_handle(root_hwnd)
        if focused and self._read_text(focused):
            return focused
        ...
        for hwnd in self._child_handles(root_hwnd):
            class_name = get_class_name(hwnd)
            if not any(hint.lower() in class_name.lower() for hint in EDITOR_CLASS_HINTS):
                continue
            text = self._read_text(hwnd)
    ...
    def read_selected_text(self) -> str:
        ...
        if not self._is_notepad(hwnd):
            return ""
        editor_hwnd = self._best_editor_handle(hwnd)
        if not editor_hwnd:
            return ""
        try:
            selection = int(win32gui.SendMessage(editor_hwnd, 0x00B0, 0, 0))
        except Exception:
            return ""
        ...
        if end <= start:
            ...
            return ""
        text = self._read_text(editor_hwnd)
        selected_text = normalize_text(text[start:end])
        self._last_selection_info = {
            "selection_mode": True,
            "selection_start": start,
            "selection_end": end,
            "selection_text": selected_text,
            "notepad_editor_handle": int(editor_hwnd),
        }
...
class ActiveHwpReader(BasePollingReader):
    ...
    def read_current_text(self) -> str:
        try:
            ...
            com_text = self._read_text_via_hwp_com(hwp) if hwp is not None else ""
            uia_text = self._read_text_via_uia(hwnd)
            text, read_method = self._choose_hwp_live_text(com_text, uia_text)
            text = self._stabilize_hwp_text(text)
            if has_text_content(text):
                self._last_read_method = read_method
                if read_method != "uia":
                    self._last_uia_source = ""
                self._last_hwp_text = text
                self._last_full_text_at = time.monotonic()
                self._last_hwp_style_info = self._read_hwp_style_info(hwp) if read_method == "com" and hwp is not None else {}
                return text
            return ""
        except Exception as exc:
    ...
    def _choose_hwp_live_text(self, com_text: str, uia_text: str) -> tuple[str, str]:
        com_text = normalize_text(com_text).strip()
        uia_text = normalize_text(uia_text).strip()
        if has_text_content(uia_text):
            if not has_text_content(com_text):
                self._log_hwp(f"HWP live selected UIA com_len=0 uia_len={len(uia_text)} preview={uia_text[:120]!r}")
                return uia_text, "uia"
            if len(uia_text) >= max(3, int(len(com_text) * 0.65)):
                if uia_text != com_text:
                    self._log_hwp(
                        "HWP live selected UIA "
                        f"com_len={len(com_text)} uia_len={len(uia_text)} "
                        f"com_preview={com_text[:120]!r} uia_preview={uia_text[:120]!r}"
                    )
                return uia_text, "uia"
            self._log_hwp(
                "HWP live kept COM because UIA looked partial "
                f"com_len={len(com_text)} uia_len={len(uia_text)} "
                f"uia_preview={uia_text[:120]!r}"
            )
        return com_text, "com"

    def _read_text_via_hwp_com(self, hwp) -> str:
        hwpml_text = self._read_hwpml2x_text(hwp, "")
        scan_text = self._scan_text(hwp)

        if has_text_content(hwpml_text):
            if not has_text_content(scan_text) or len(hwpml_text.strip()) >= max(1, len(scan_text.strip()) - 2):
                if hwpml_text != scan_text:
                    self._log_hwp(
                        "HWP COM selected HWPML2X "
                        f"hwpml_len={len(hwpml_text)} scan_len={len(scan_text)} "
                        f"hwpml_preview={hwpml_text[:120]!r} scan_preview={scan_text[:120]!r}"
                    )
                return hwpml_text

        return scan_text

    def _read_hwpml2x_text(self, hwp, option: str = "") -> str:
        getter = getattr(hwp, "GetTextFile", None)
        if not callable(getter):
            return ""
        try:
            xml_text = getter("HWPML2X", option)
        except Exception as exc:
            self._log_hwp(f"HWPML2X GetTextFile option={option!r} failed: {type(exc).__name__}: {exc}")
            return ""
        return self._extract_hwpml2x_plain_text(str(xml_text or ""))
    ...
    def _extract_hwpml2x_plain_text(self, xml_text: str) -> str:
        ...
        try:
            ...
            self._log_hwp(f"HWPML2X parse failed: {type(exc).__name__}: {exc}")
            return ""
        ...
        for paragraph in root.findall(".//BODY//P"):
            ...
            for text_node in paragraph.findall("TEXT"):
                for char_node in text_node.findall("CHAR"):
                    if char_node.text:
                        chunks.append(html.unescape(char_node.text))
    ...
    def read_selected_text(self) -> str:
        try:
            ...
            if not self._is_hwp_window(hwnd):
                return ""
            hwp = self._active_hwp()
            text = self._read_selected_text_via_hwpml2x(hwp) if hwp is not None else ""
            if not has_text_content(text):
                text = self._read_selected_text_via_clipboard()
            if has_text_content(text):
                self._remember_hwp_selection_info(text)
                return text.strip()
        except Exception as exc:
            self._debug(f"hwp selected read failed: {exc}")
        return ""
    ...
    def _read_selected_text_via_hwpml2x(self, hwp) -> str:
        ...
        if has_text_content(text):
            ...
            if not has_text_content(full_text):
                try:
                    full_text = normalize_text(self._read_text_via_hwp_com(hwp)).strip()
                except Exception:
                    full_text = ""
            # Some HWP builds return full-document text even with option="selection".
            # In that case, force fallback to clipboard selection capture.
            if has_text_content(full_text):
                same_as_full = normalized == full_text
                almost_full = len(normalized) >= int(len(full_text) * 0.95) and full_text.startswith(
                    normalized[: max(1, min(24, len(normalized)))]
                )
    ...
    def _remember_hwp_selection_info(self, selected_text: str):
        normalized_selection = normalize_text(selected_text).strip()
        ...
        if start_index < 0:
            ...
            return
        end_index = start_index + len(normalized_selection)
        self._last_selection_info = {
            "selection_mode": True,
            "selection_text": normalized_selection,
            "hwp_selection_start_index": start_index,
            "hwp_selection_end_index": end_index,
            "hwp_selection_start_pos": self._hwp_text_index_to_position(normalized_full, start_index),
            "hwp_selection_end_pos": self._hwp_text_index_to_position(normalized_full, end_index),
            "hwp_selection_length": len(normalized_selection),
        }

    def _read_selected_text_via_clipboard(self) -> str:
        if pyperclip is None or win32api is None or win32con is None:
            return ""
        sentinel = f"__AI_GRAMMARY_NO_SELECTION_{time.monotonic_ns()}__"
        try:
        ...
        try:
            ...
            self._send_ctrl_c()
            time.sleep(0.12)
            copied = normalize_text(pyperclip.paste())
            if not copied or copied == sentinel or "AI_GRAMMARY_NO_SELECTION_" in copied:
                return ""
            return copied
        except Exception as exc:
            self._debug(f"hwp selected clipboard read failed: {exc}")
            return ""
        finally:
    ...
    def _stabilize_hwp_text(self, text: str) -> str:
        normalized = normalize_text(text)
        if not has_text_content(normalized):
            return normalized
        previous = normalize_text(self._last_hwp_text)
        if not has_text_content(previous):
            return normalized
        if normalized == previous:
            return normalized
        if self._is_suspicious_partial_hwp_snapshot(previous, normalized):
            self._log_hwp(
                "HWP partial snapshot ignored "
                f"previous_len={len(previous)} current_len={len(normalized)} "
                f"previous_newlines={previous.count(chr(10))} current_newlines={normalized.count(chr(10))}"
            )
            return previous
        return normalized

    def _is_suspicious_partial_hwp_snapshot(self, previous: str, current: str) -> bool:
        if len(current) >= len(previous):
            return False
        if len(current) < max(32, int(len(previous) * 0.9)):
            if current in previous:
                return True
            previous_lines = [line.strip() for line in previous.split("\n") if line.strip()]
            current_lines = [line.strip() for line in current.split("\n") if line.strip()]
            if current_lines and previous_lines:
                matches = sum(1 for line in current_lines if line in previous_lines)
                if matches >= max(2, len(current_lines) - 1):
                    return True
        return False
...
class UniversalActiveTextReader:
    def __init__(self, debug: bool = False):
        ...
        self.last_signatures: dict[str, tuple[str, str, str]] = {}
        self.last_style_signatures: dict[str, str] = {}
    ...
    def _style_fingerprint(self, style_info: dict | None) -> str:
        if not style_info:
            return ""
        try:
            return json.dumps(style_info, ensure_ascii=False, sort_keys=True, default=str)
        except Exception:
            return str(style_info)

    def poll_snapshot(self, selection_only: bool = False) -> ReaderSnapshot | None:
        source_name = "selection" if selection_only else "realtime"
        ...
        for reader_name, reader in self._reader_order_for_foreground():
            ...
            if signature == self.last_signatures.get(source_name):
                style_info = self._read_style_info(reader)
                if selection_only:
                    style_info = dict(style_info)
                    selection_info_reader = getattr(reader, "read_selection_info", None)
                    if callable(selection_info_reader):
                        style_info.update(selection_info_reader() or {})
                    style_info["selection_mode"] = True
                style_signature = self._style_fingerprint(style_info)
                if style_signature and style_signature != self.last_style_signatures.get(source_name, ""):
                    self.last_style_signatures[source_name] = style_signature
            ...
            if selection_only:
                ...
                if callable(selection_info_reader):
                    style_info.update(selection_info_reader() or {})
                style_info["selection_mode"] = True
            self.last_signatures[source_name] = signature
            self.last_style_signatures[source_name] = self._style_fingerprint(style_info)
            self._remember_active_snapshot(reader_name, hwnd, window_title)
            return ReaderSnapshot(
                source=source_name,
                window_title=window_title,
        ...
        if window_title and empty_signature != self.last_signatures.get("realtime"):
            self.last_signatures["realtime"] = empty_signature
            return ReaderSnapshot(
                source="realtime",
                window_title=window_title,
                text="",
